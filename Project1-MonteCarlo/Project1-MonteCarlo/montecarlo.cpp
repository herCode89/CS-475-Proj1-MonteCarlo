...
//#define NUMT	1
//#define NUMT	2
//#define NUMT	4          //(Created for individual testing)
//#define NUMT	6
//#define NUMT	8
#define NUMT	12
#endif
...
//#define NUMTRIALS	1000            //(Created for individual testing)
...
const float minTX = -10; // truck starting location in feet
const float maxTX = 10; // truck starting location in feet

const float minTXV = 10; // truck x velocity in feet/sec
const float maxTXV = 30; // truck x velocity in feet/sec

const float minTY = 45; // depth distance to truck in feet
const float maxTY = 55; // depth distance to truck in feet

const float minSV = 10; // snowball velocity in feet/sec
const float maxSV = 30; // snowball velocity in feet/sec

const float minTHETA = 10; // snowball launch angle in degrees
const float maxTHETA = 90; // snowball launch angle in degrees
...
main(int argc, char* argv[])
{
    ...
    {
        txs[n] = Ranf(minTX, maxTX);
        tys[n] = Ranf(minTY, maxTY);
        txvs[n] = Ranf(minTXV, maxTXV);
        svs[n] = Ranf(minSV, maxSV);
        sths[n] = Ranf(minTHETA, maxTHETA);
    }
...
// a different random number sequence every time you run it:  (This portion of the code was removed from bottom and placed at top)
